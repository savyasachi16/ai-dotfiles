...
Push the current branch as-is after a commit audit.

Steps:
1. Check the working tree with `git status --short`.
2. If the tree is dirty, run the `/commit` flow first. If the dirty work cannot be safely committed, stop and report what blocks it.
...
4. Run relevant tests/typecheck for the current commit set. If they fail, surface the failure and ask before pushing.
