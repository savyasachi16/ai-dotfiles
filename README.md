...
Currently available: **`/handoff`** (seal session into `.ai/journal.md`, optionally promote to tracked `## Decisions` in `AI.md`), **`/catchup [N]`** (replay last N journal entries + durable decisions, end with "what's the move?"), **`/commit`** (commit the current logical unit with Conventional Commits), **`/push`** (run the docs/instructions audit, then push the current branch), and **`/configure-agents`** (fetch official docs for all 4 tools, propose a cross-agent settings change in the correct format for each, execute after approval). The commands use root `AI.md` in normal repos and fall back to `instructions/AI.md` for this dotfiles repo. See `## Session continuity` and `## Commit cadence` in `instructions/AI.md` for the full protocol.
